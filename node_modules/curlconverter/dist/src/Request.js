...
function buildURL(global, config, url, uploadFile, stdin, stdinFile) {
...
function buildData(configData, stdin, stdinFile) {
    ...
    const dataStr = mergeWords(data.map((d) => {
        if (Array.isArray(d)) {
            const name = d[1];
            const filename = d[2];
            dataStrReadsFile || (dataStrReadsFile = filename.toString()); // report first file
            if (name) {
...
function buildRequest(global, config, stdin, stdinFile) {
    ...
    for (let [i, originalUrl] of config.url.entries()) {
        ...
        const [urlObj, url, queryReadsFile, urlWithoutQueryList, queryList, queryDict, urlWithoutQueryArray, urlWithOriginalQuery, queryArray, urlQueryArray,] = buildURL(global, config, originalUrl, uploadFile, stdin, stdinFile);
...
export function buildRequests(global, stdin, stdinFile) {
