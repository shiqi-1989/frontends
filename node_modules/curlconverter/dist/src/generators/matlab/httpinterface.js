import { reprStr, repr, setVariableValue, callFunction, structify, containsBody, prepareQueryString, prepareCookies, } from "./common.js";
...
function prepareMultipartUploads(request) {
    ...
    for (const m of request.multipartUploads) {
        ...
        params.push([repr(m.name), fileProvider]); // TODO: can be a string[]
    }
...
function prepareDataProvider(value, output, termination, indentLevel, isDataBinary = true, isDataRaw = false) {
    if (!isDataRaw && value.charAt(0) === "@") {
        ...
        const isImageProvider = new Set(["jpeg", "jpg", "png", "tif", "gif"]).has(filename.split(".")[1].toString());
...
function prepareData(request) {
    ...
    if (request.data && request.data.split("&", 2).length > 1) {
        const data = request.data.split("&").map((x) => x.split("=").map((x) => {
            let ans = repr(x);
            if (x.isString()) {
                try {
...
export function toHTTPInterface(request, warnings) {
    return [
        [
            "%% HTTP Interface",
            "import matlab.net.*",
