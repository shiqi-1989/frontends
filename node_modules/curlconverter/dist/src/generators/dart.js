...
function repr(value, imports) {
    ...
    for (const t of value.tokens) {
        if (typeof t === "string") {
            ...
            ret.push("(await Process.run(" + reprStr(t.value) + ", runInShell: true)).stdout");
            imports.add("dart:io");
        }
    }
    return ret.join(" + ");
...
export function _toDart(requests, warnings = []) {
    ...
    const queryIsRepresentable = request.urls[0].queryList &&
    ...
    if (queryIsRepresentable && request.urls[0].queryList) {
        ...
        for (const [paramName, rawValue] of request.urls[0].queryList) {
            const paramValue = repr(rawValue !== null && rawValue !== void 0 ? rawValue : new Word(), imports);
