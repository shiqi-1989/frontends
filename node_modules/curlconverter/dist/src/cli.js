...
import { _toJavaScript, toJavaScriptWarn, } from "./generators/javascript/javascript.js";
...
import { _toNodeAxios, toNodeAxiosWarn, } from "./generators/javascript/axios.js";
import { _toNodeGot, toNodeGotWarn } from "./generators/javascript/got.js";
import { _toNodeRequest, toNodeRequestWarn, } from "./generators/javascript/request.js";
import { _toPhp, toPhpWarn } from "./generators/php/php.js";
import { _toPhpRequests, toPhpRequestsWarn, } from "./generators/php/requests.js";
...
const translate = {
    ...
    "c#": [_toCSharp, toCSharpWarn],
    browser: [_toJavaScript, toJavaScriptWarn],
    dart: [_toDart, toDartWarn],
    elixir: [_toElixir, toElixirWarn],
    go: [_toGo, toGoWarn],
    golang: [_toGo, toGoWarn],
    java: [_toJava, toJavaWarn],
    javascript: [_toJavaScript, toJavaScriptWarn],
    "javascript-fetch": [_toJavaScript, toJavaScriptWarn],
    "javascript-axios": [_toNodeAxios, toNodeAxiosWarn],
    "javascript-got": [_toNodeGot, toNodeGotWarn],
    "javascript-request": [_toNodeRequest, toNodeRequestWarn],
    json: [_toJsonString, toJsonStringWarn],
    matlab: [_toMATLAB, toMATLABWarn],
    node: [_toNode, toNodeWarn],
    "node-fetch": [_toNode, toNodeWarn],
    "node-axios": [_toNodeAxios, toNodeAxiosWarn],
    "node-got": [_toNodeGot, toNodeGotWarn],
    "node-request": [_toNodeRequest, toNodeRequestWarn],
    nodejs: [_toNode, toNodeWarn],
    "nodejs-fetch": [_toNode, toNodeWarn],
    "nodejs-axios": [_toNodeAxios, toNodeAxiosWarn],
    "nodejs-got": [_toNodeGot, toNodeGotWarn],
    "nodejs-request": [_toNodeRequest, toNodeRequestWarn],
...
const longOpts = { ...curlLongOpts, ...curlconverterLongOpts };
const shortOpts = { ...curlShortOpts, ...curlconverterShortOpts };
...
function exitWithError(error, verbose = false) {
    let errMsg = error;
    if (!verbose) {
...
if (!has(translate, language)) {
    exitWithError(new CCError("unexpected --language: " +
        JSON.stringify(language) +
        "\n" +
        "must be one of: " +
        Object.keys(translate).sort().join(", ")), verbose);
}
...
if (commandFromStdin) {
    ...
    if (extraArgs.length > 0) {
        ...
        exitWithError(new CCError("if you pass --stdin or -, you can't also pass " + extraArgsStr), verbose);
    }
